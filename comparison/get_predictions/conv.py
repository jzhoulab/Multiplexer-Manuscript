...
Conv.load_state_dict(torch.load('../../data/comparison/weights/Conv.pth'))
Conv.eval()
...
#start the loop here
#
C = torch.zeros((size, 2002, 4, 2000))
...
for i in range(size):
    ...
    ####Conv section####
    C_output = Conv(input).detach().cpu() #Shape is [1, 2002, 4, 2000]
...
torch.save(C, "../predictions/Conv_predictions")
