...
class FullInterpreter(nn.Module):
    def __init__(self):
        super(FullInterpreter, self).__init__()
            ...
            for x,y in val_data:
                with torch.no_grad():
                    ...
                    if loss.item() < min_validation_loss:
                        ...
                        torch.save(model.state_dict(), "ConvUnet.pth")
...
FI = FullInterpreter()
FI.cuda()
FI = nn.DataParallel(FI)


loss_function = nn.MSELoss()
optimizer = torch.optim.Adam(FI.parameters(), lr = 0.0001)    
f = train( val_loader, FI, Beluga_model, optimizer, loss_function, 99999999) 